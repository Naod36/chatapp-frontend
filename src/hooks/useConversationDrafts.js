...
export function useConversationDrafts(accountId, onStorageError) {
  const [savedStore, setStore] = useState(() =>
    createDraftStore(accountId, localStorage),
  );
  ...
  const changed = (persisted) => {
    ...
    if (!persisted)
      onStorageError(
        "Draft kept in this tab, but could not be saved for refresh.",
      );
  };
  return {
    drafts: store.drafts,
    ...
    moveDraft: (from, to) => {
      ...
      changed(
        store.set(to, store.drafts[to] ? `${store.drafts[to]}\n${text}` : text),
      );
