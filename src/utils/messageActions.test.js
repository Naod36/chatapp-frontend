...
import {
  createMessageActionTracker,
  matchesMessageAction,
...
test("confirmation must match conversation, message, action, and actor", () => {
  ...
  for (const difference of [
    ...
    assert.equal(
      matchesMessageAction(action, { ...event, ...difference }, "me"),
      false,
    );
  }
  assert.equal(
    matchesMessageAction(
      { ...action, action: "edit_message", content: "new" },
      { ...event, event: "message_edited", content: "old" },
      "me",
    ),
    false,
  );
  assert.equal(
    matchesMessageAction(
      { ...action, action: "pin_message", scope: "personal" },
      {
        ...event,
  ...
  for (const [request, response] of [
    ...
    assert.equal(
      matchesMessageAction(
        pin,
        { ...confirmation, pinned_by_user_id: "other" },
        "me",
      ),
      false,
    );
...
test("one pending action settles once; failures and timeouts retain recovery context", () => {
  ...
  const tracker = createMessageActionTracker({
    ...
    onSettle: (...args) => settled.push(args),
    schedule: (callback) => {
      expire = callback;
    },
    cancel() {},
  ...
  assert.match(
    tracker.start({ ...action, message_id: "temp-local" }),
    /not been confirmed/,
  );
  connected = true;
  tracker.start(action);
