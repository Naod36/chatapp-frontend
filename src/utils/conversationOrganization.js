export function organizedConversations(conversations, organization, view) {
  ...
  if (view === "archived") return conversations.filter((conversation) => archived.has(conversation.id));
  if (view.startsWith("folder:")) {
    const folder = organization?.folders.find((item) => item.id === view.slice(7));
