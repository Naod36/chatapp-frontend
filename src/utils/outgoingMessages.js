export function confirmOutgoingMessage(messages, temporaryId, confirmation) {
  ...
  const echoed = messages.find(
    (message) => (message.message_id || message.id) === confirmation.message_id,
  );
  const confirmed = {
    ...optimistic,
    ...echoed,
    ...
    status:
      echoed?.status === "read" || echoed?.status === "delivered"
        ? echoed.status
        : confirmation.status,
  };
  return messages.flatMap((message) => {
    if (message.id === temporaryId) return [confirmed];
    if ((message.message_id || message.id) === confirmation.message_id)
      return [];
    return [message];
  });
}
...
export function isConfirmedMessage(message) {
  return (
    message &&
    !String(message.id || message.message_id).startsWith("temp-") &&
