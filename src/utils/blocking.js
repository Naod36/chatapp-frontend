...
export function directBlockPolicy(
  conversation,
  blockedUserIds,
  blockedByUserIds,
) {
...
export function maskParticipant(participant, blockedByUserIds) {
  if (
    !participant ||
    !blockPolicy(participantId(participant), [], blockedByUserIds).maskIdentity
  ) {
    return participant;
  }
  return {
...
export function maskConversation(conversation, blockedByUserIds) {
  ...
  const hidden = directBlockPolicy(
    conversation,
    [],
    blockedByUserIds,
  ).maskIdentity;
  return {
    ...
    ...(hidden
      ...
      : {}),
    other_participant: maskParticipant(
...
export function visibleTypingUsers(typingUsers, blockedByUserIds) {
  return Object.fromEntries(
    Object.entries(typingUsers).map(([conversationId, typists]) => [
      conversationId,
      Object.fromEntries(
        Object.entries(typists).filter(
