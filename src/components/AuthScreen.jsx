...
const EyeIcon = ({ color, hidden = false }) => (
  <svg
    width="18"
    height="18"
    viewBox="0 0 24 24"
  ...
  >
    {hidden ? (
      <>
        ...
        <path d="M9.88 4.24A9.77 9.77 0 0 1 12 4c5 0 8.27 4 9 5a11.44 11.44 0 0 1-2.17 2.63" />
        <path d="M6.61 6.61C4.62 7.95 3.29 9.58 3 10c.73 1 4 5 9 5a9.77 9.77 0 0 0 2.12-.24" />
      </>
    ) : (
      <>
        <path d="M2.06 12.35a1 1 0 0 1 0-.7C3.43 8.14 7.24 5 12 5c4.76 0 8.57 3.14 9.94 6.65a1 1 0 0 1 0 .7C20.57 15.86 16.76 19 12 19c-4.76 0-8.57-3.14-9.94-6.65Z" />
...
const GoogleIcon = () => (
  ...
  >
    ...
    />
    <path
      d="M5.84 14.1c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.08H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.92l2.85-2.22c-.87-2.6-2.87-4.6-6.19-4.6z"
      fill="#FBBC05"
    />
    <path
...
export default function AuthScreen({
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.lineWidth = 2.6;
      ctx.strokeStyle =
        theme === "light" ? "rgba(0, 0, 0, 0.08)" : "rgba(255, 255, 255, 0.09)";
      ...
      lines.forEach((line) => {
        ...
        for (let x = 0; x <= w; x += 15) {
          const baseOffsetY =
            centerY + Math.sin(x * line.frequency + linePhase) * line.amplitude;
    ...
    function draw() {
      ...
      for (let y = spacing / 2; y < h; y += rowHeight) {
        ...
        for (let x = -spacing + xOffset; x < w + spacing; x += spacing) {
          ...
          const sX = drawX * smokeScale - now * -0.0005;
          const sY = drawY * smokeScale + now * 0.00035;

          // Four wave octaves to generate chaotic plumes with zero repetition
          const n1 =
            Math.sin(sX * 1.0 + sY * 0.8) * Math.cos(sY * 1.2 - sX * 0.4);
          const n2 =
            Math.cos(sX * 2.3 - sY * 1.6) * Math.sin(sY * 1.9 + sX * 0.7);
          const n3 =
            Math.sin(sX * 4.8 + sY * 3.2) * Math.cos(sY * 5.1 - sX * 2.8);
          const n4 =
            Math.sin(sX * 10.5 - sY * 8.0) * Math.cos(sY * 9.2 + sX * 4.5);

          const noiseFlow = n1 * 0.46 + n2 * 0.28 + n3 * 0.16 + n4 * 0.1;
          ...
          if (mouse.current.active) {
            ...
            if (mdist < hoverRadius) {
              intensity = Math.min(
                1.4,
                intensity + (1 - mdist / hoverRadius) * 0.52,
              );
            }
          }
          ...
          for (const rip of ripples.current) {
            ...
            if (Math.abs(rdist - ripRadius) < ringWidth) {
              const strength =
                (1 - age / 1000) *
                (1 - Math.abs(rdist - ripRadius) / ringWidth);
      ...
      >
        ...
        >
          ...
          <div className="fx-stagger flowchat-brand-wrapper">
            ...
            >
              <path
                className="flowchat-icon-bar1"
                d="M5.5 17L9.5 7H13.5L9.5 17H5.5Z"
                fill="currentColor"
              />
              <path
                className="flowchat-icon-bar2"
                d="M12.5 17L16.5 7H20.5L16.5 17H12.5Z"
                fill="currentColor"
              />
            </svg>
            <span className={`flowchat-brand-name ${theme}`}>FlowChat</span>
          </div>
          ...
          {(mode === "signin" || mode === "signup") && (
            <>
              ...
              >
                <div
                  style={{ flex: 1, height: 1, background: t.inputBorder }}
                />
                <span
                  style={{
                    fontSize: 11,
          ...
          {mode === "signup" && (
            <>
              <label className="fx-stagger" style={labelStyle(t)}>
                Username
              </label>
              <input
                className="fx-stagger"
          ...
          {!mode.endsWith("-sent") && mode !== "reset-complete" && (
            <button
              type="submit"
              disabled={loading}
              className="fx-stagger"
  ...
  return (
    ...
    >
      <input
        ...
        required
        style={{ ...inputStyle(t), marginBottom: 0, paddingRight: 52 }}
      />
      <button
        type="button"
      ...
      >
        <EyeIcon color={t.textMuted} hidden={showPassword} />
      </button>
    </div>
  );
}
