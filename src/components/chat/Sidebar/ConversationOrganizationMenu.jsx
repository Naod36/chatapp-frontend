...
export default function ConversationOrganizationMenu({
  ...
  const mutate = async (change) => {
    if (await organization.update(change)) onClose();
  };
  ...
  return createPortal(
    <div
      ...
      className={`ht-conversation-menu ht-${theme}-theme`}
      style={{
        left: request.x,
        top: request.y,
    ...
    >
        ...
        return (
          ...
          >
            <svg
              ...
              aria-hidden="true"
            >
              <path d={included ? "m5 12 4 4L19 6" : "M3 6h7l2 2h9v12H3z"} />
            </svg>
      ...
      {organization.error && (
        <div role="alert" className="ht-menu-feedback">
          {organization.error}
        </div>
      )}
      {organization.pending && (
        <div role="status" className="ht-menu-feedback">
          Saving...
        </div>
      )}
      <div className="ht-menu-separator" role="separator" />
      <button
        type="button"
        role="menuitem"
        disabled={!organization.ready || organization.pending}
        onClick={onCreate}
      >
        <svg
          width="17"
