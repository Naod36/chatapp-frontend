            ...
            {activeConv && (
                <div className="ht-inspector-inner">
                    ...
                    <div className="ht-inspector-actions" style={{ borderBottom: t.border, paddingBottom: 20 }}>
                        ...
                        <button
                            className="ht-inspector-action-btn"
                            style={{ color: (Array.isArray(mutedConvIds) ? mutedConvIds.includes(activeConv.id) : mutedConvIds?.[activeConv.id]) ? "#ef4444" : t.text }}
                            onClick={() => toggleMuteConversation(activeConv.id)}
                            title={(Array.isArray(mutedConvIds) ? mutedConvIds.includes(activeConv.id) : mutedConvIds?.[activeConv.id]) ? "Unmute Notifications" : "Mute Notifications"}
                        >
                            {(Array.isArray(mutedConvIds) ? mutedConvIds.includes(activeConv.id) : mutedConvIds?.[activeConv.id]) ? (
                            ...
                            <span>{(Array.isArray(mutedConvIds) ? mutedConvIds.includes(activeConv.id) : mutedConvIds?.[activeConv.id]) ? "Muted" : "Mute"}</span>
                        </button>
                        <button
                            className="ht-inspector-action-btn"
