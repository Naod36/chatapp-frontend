    ...
    return (
      <div className="ht-chat-pane">
        ...
        >
          ...
          <p
            style={{ margin: 0, fontSize: 13, maxWidth: 280, lineHeight: 1.45 }}
          >
            Choose a message thread from the inbox sidebar, or search profiles
  ...
  return (
    <div className="ht-chat-pane">
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    color: t.text,
                    cursor: "pointer",
                    opacity: searchMatchingMessages.length ? 1 : 0.3,
                    padding: "2px 4px",
                  }}
          ...
          <div
            ...
            title={
              activeConv.type === "group"
                ? "Click to view group info & settings"
          ...
          <div ref={headerMenuRef} style={{ position: "relative" }}>
            ...
            {isHeaderMenuOpen && (
              ...
              >
                ...
                >
                  {(
                    Array.isArray(mutedConvIds)
                      ? mutedConvIds.includes(activeConv.id)
      ...
      {(() => {
        ...
        const pinnedMsg =
          messages.find(
            (m) => String(m.id || m.message_id) === String(pinnedMsgId),
          ) || pinItem;
        const isSelf = pinnedMsg.sender_id === user.userId;
        const senderPart = (activeConv?.participants || []).find(
          (p) => (p.user_id || p.id) === pinnedMsg.sender_id,
        );
        const senderTitle = isSelf
          ? "You"
          : pinnedMsg.sender_name ||
            senderPart?.display_name ||
            senderPart?.username ||
            activeConv.display_name ||
            "Participant";
        const scopeBadge =
          pinItem.scope === "personal" ? " (Only for you)" : "";
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: t.accent }}>
                  Pinned Message{" "}
                  {pins.length > 1 ? `(${safeIndex + 1}/${pins.length})` : ""} •{" "}
                  {senderTitle}
                  {scopeBadge}
                </div>
                <div
      ...
      >
        {messages.map((m) => {
          ...
          const parentMsg = m.reply_to_id
            ? messages.find(
                (msg) => (msg.id || msg.message_id) === m.reply_to_id,
              )
            : null;
          ...
          const isCurrentSearchMatch =
            isSearchMatched &&
            searchMatchingMessages[inChatSearchMatchIndex]?.id ===
              (m.id || m.message_id);
                ...
                >
                  {activeConv.type === "group" && !isSelf && (
                    ...
                    >
                      {(() => {
                        ...
                        ).find((p) => (p.user_id || p.id) === m.sender_id);
                        return (
                          m.sender_name ||
                          senderPart?.display_name ||
                          senderPart?.username ||
                  ...
                  {m.message_type === "image" ? (
                    ...
                    >
                      ...
                      {m.content &&
                        m.content !==
                          (m.media_url || m.file_url)?.split("/").pop() && (
                          <div
                            style={{
                              padding: "4px 8px 2px",
                              fontSize: 13,
                  ...
                  >
                    {Object.entries(m.reactions).map(([emoji, uids]) => {
                      ...
                      const reactUsers = uids.map((uid) => {
                        ...
                        const part = (activeConv?.participants || []).find(
                        ...
                        if (
                          other &&
                          String(other.user_id || other.id) === normUid
                        ) {
                          return {
                            avatar: other.avatar_url,
                            initial: (other.display_name ||
                              other.username ||
                              "?")[0].toUpperCase(),
                          };
                        }
                            ...
                            {shownUsers.map((u, idx) => (
                              <div
                                ...
                                style={{
                                  ...
                                  background: u.avatar
                                    ? "transparent"
                                    : "linear-gradient(135deg, #4A3FE0, #8B7FFF)",
      ...
      {contextMenu && (
        ...
        >
          ...
          >
            {(pinnedMessagesMap?.[activeConv?.id] || []).some(
              (p) =>
                String(p.message_id || p.id) ===
                String(
                  contextMenu.message.id || contextMenu.message.message_id,
                ),
            )
              ? "Unpin Message"
              : "Pin Message"}
