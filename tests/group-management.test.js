...
test("group changes wait for confirmation, prevent duplicate writes, retain failure and retry", async () => {
  ...
  try {
    ...
    await act(async () => { actions.value = "remove_member"; actions.dispatchEvent(new window.Event("change", { bubbles: true })); });
    assert.deepEqual(changes, []);
    window.confirm = () => true;
    await act(async () => { actions.value = "transfer_ownership"; actions.dispatchEvent(new window.Event("change", { bubbles: true })); });
