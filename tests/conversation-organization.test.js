...
test("organization views exclude archives from inbox and preserve folder membership", () => {
  const chats = [
    { id: "direct", unread_count: 2 },
    { id: "group", type: "group" },
  ];
  const state = {
    archived_ids: ["direct"],
    folders: [{ id: "work", conversation_ids: ["direct", "group"] }],
  };
  ...
  assert.deepEqual(organizedConversations(chats, state, "archived"), [
    chats[0],
  ]);
...
test("organization confirms mutations, ignores stale responses, refreshes across clients and isolates sessions", async () => {
  ...
  const { default: useOrganization } = await server.ssrLoadModule(
    "/src/hooks/useConversationOrganization.js",
  );
  const { organizationService } = await server.ssrLoadModule(
    "/src/services/organization.js",
  );
  const { default: OrganizationControls } = await server.ssrLoadModule(
    "/src/components/chat/Sidebar/OrganizationControls.jsx",
  );
  ...
  organizationService.update = (change) =>
    new Promise((resolve, reject) => changes.push({ change, resolve, reject }));
  const snapshot = (revision, archived_ids = []) => ({
    revision,
    archived_ids,
    folders: [],
  });
  ...
  try {
    ...
    await act(async () => {
      current.refresh();
      current.update({ action: "delete_folder", folder_id: "folder" });
    });
    assert.equal(requests.length, 2);
    assert.equal(changes.length, 1, "parallel mutations are disabled");
    await act(async () => {
      changes[0].resolve(snapshot(1, ["chat"]));
      await mutation;
    });
    await act(async () => requests[1].resolve(snapshot(0)));
    assert.deepEqual(
      current.archived_ids,
      ["chat"],
      "old GET cannot undo mutation",
    );
    await act(async () => {
      mutation = current.update({
        action: "archive",
    ...
    await act(async () => {
      changes[1].reject(new Error("offline"));
      await mutation;
    });
    ...
    assert.deepEqual(
      current.archived_ids,
      [],
      "focus sees other client's restore",
    );
    ...
    await act(async () => {
      changes[2].resolve(snapshot(3, ["chat"]));
      await mutation;
      oldSession.refresh();
    });
    ...
    assert.deepEqual(
      current.archived_ids,
      ["other"],
      "refresh errors preserve known organization",
    );
    ...
    let controlState = {
      ...
      pending: false,
      error: null,
      folders: [{ id: "work", name: "Work", conversation_ids: [] }],
    };
    ...
    const renderControls = () =>
      root.render(
        React.createElement(OrganizationControls, {
          ...
          themeTokens: { text: "#222", sidebarBg: "#fff" },
        }),
      );
    ...
    await click(document.querySelector('[aria-label="Manage folders"]'));
    ...
    controlState = {
      ...controlState,
      archived_ids: ["chat"],
      folders: [{ id: "work", name: "Work", conversation_ids: ["chat"] }],
    };
    await act(async () => renderControls());
    await click(document.querySelector('[aria-label="Include Peer in Work"]'));
